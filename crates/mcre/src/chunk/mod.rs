...
#[derive(Asset, Clone, Debug, TypePath, Deserialize, Serialize, Component)]
pub struct Chunk {
...
/// Convert world position to chunk position and local block position within chunk
pub fn world_pos_to_chunk_pos(world_pos: IVec3, chunk_size: &ChunkSize) -> (ChunkPosition, UVec3) {
    let chunk_pos = chunk_size.chunk_coord(world_pos.as_vec3());
    let chunk_world_coord = chunk_pos.world_coord(*chunk_size);
    let chunk_local_pos = (world_pos.as_vec3() - chunk_world_coord).as_uvec3();
